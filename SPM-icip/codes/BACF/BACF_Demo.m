...
%=======================SPM INTEGRATION=======================%
%Note: please change the base_path to the directory where you put our dataset in.
base_path  = '../../dataset';
videos = {'1_car5','1_nonm7','1_nonm12','2_car3','2_people9','5_nonm1','5_people2','6_people2','6_people4','6_people8','7_nonm2','7_nonm3','7_people6','8_people2','8_people4','seq1','seq2','seq3'};
%=======================SPM INTEGRATION=======================%
...
for vid = 1:numel(videos)
    ...
    save_name = ['S-BACF_',seq.VidName,'.txt'];
    fw = fopen(save_name,'w');
    rows = size(res, 1);
    cols = size(res, 2);
    for i = 1:rows
        for j=1:cols - 1
            fprintf(fw, '%f ', res(i,j));
        end    
        fprintf(fw, '%f\n', res(i, cols));  % for Linux
    end
    fclose(fw);
