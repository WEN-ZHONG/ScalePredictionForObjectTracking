function [ result ] = CREST_tracking( opts, varargin, config, display, test_seq)
% Test function
global objSize;
...
for i=2:nFrame       
 ...
 %=======================SPM INTEGRATION=======================%
    %% Original scale estimation
%      target_szU=scale_estimation_1(im,pos,target_szU,window_sz,...
%              net1,net_online,coeff,meanImg,featPCA1st); 

%%% Scale Estimation using SPM
    x1 = pos(2)/scale;
    y1 = pos(1)/scale;
    [h1, scaleRatio] = scale_estimation(theta, x1, y1, x0, y0, h0);
    w1 = w0*scaleRatio;
    target_szU = [h1*scale w1*scale];
%=======================SPM INTEGRATION=======================%
...
% %         warning off;
% %         yyyy = '/media/wangxiao/49cd8079-e619-4e4b-89b1-15c86afb5102/results_of_compare_methods/CREST/';
% %         saveTracking_path = [yyyy, test_seq, '/'];
% %         mkdir(saveTracking_path);
% %         saveName = [test_seq, '_', num2str(i), '_CREST_.png'];
% %         xxxx = [saveTracking_path, saveName];
% %         saveas(gcf,xxxx);
