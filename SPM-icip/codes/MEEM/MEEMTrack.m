...
function result = MEEMTrack(input, ext, show_img, init_rect, start_frame, end_frame,theta)

%=======================SPM INTEGRATION=======================%
x0 = init_rect(1)+init_rect(3)/2;
y0 = init_rect(2)+init_rect(4)/2;
w0 = init_rect(3);
h0 = init_rect(4);
%=======================SPM INTEGRATION=======================%
...
%=======================SPM INTEGRATION=======================%
    x = res(1)/config.image_scale;
    y = res(2)/config.image_scale;
    [h, scale] = scale_estimation(theta, x, y, x0, y0, h0);
    w = w0*scale;
    res(3) = w*config.image_scale;
    res(4) = h*config.image_scale;
%=======================SPM INTEGRATION=======================%
    
    result.res(frame_id-start_frame+1,:) = res/config.image_scale;
    disp(res/config.image_scale);
end
